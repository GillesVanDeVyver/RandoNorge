...
//
// WAS `index.tsx`, AND THAT WAS THE MISMATCH THIS PHASE IS ABOUT. On the web,
// `/alpha/` is AccountOverview — a hub whose cards open this list at `/saved`.
// On the phone this list WAS the landing screen, so the two clients disagreed
// about what the root of a signed-in session is: the phone had no hub, and the
// web's `/saved` had no counterpart. The screen itself is unchanged by the move
// apart from its toolbar, which follows the account actions to the hub.
//
// It also proved the phone could authenticate against the real backend and read
// real data, which is what the previous phase was judged on. Nothing about that
// depended on it being the root.
//
...
export default function RoutesScreen() {
  ...
  return (
    <View style={styles.page}>
      {/* No toolbar. The language switcher and sign-out were here because this
          was the root screen and there was nowhere else for them to be; they
          are account-level actions and now live on the account hub, which is
          also where the web keeps them. What names this screen is the stack
          header's title (see _layout.tsx), the way the header names every other
          pushed screen. */}
