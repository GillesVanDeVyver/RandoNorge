...
function pointFrom(el: XmlElement): LatLng | null {
...
function toSegment(points: XmlElement[]): Segment | null {
  const seg: Segment = [];
...
/** A file that can be read as text. A browser `File` satisfies this. */
 ...
 *
 * The parameter is structural rather than `File` on purpose: that is the whole
 * of the adapter. A browser `File` satisfies TextFile as it stands, so every
 * call site in the web app is unchanged, while the parser above — and this
 * package's tsconfig, which has no DOM lib — never has to know what a File is.
 * On React Native, whatever the document picker returns is wrapped in an object
 * with a `text()`; see ./import.ts, which needs one member more.
 */
export async function importGpxFile(file: TextFile): Promise<Route> {
